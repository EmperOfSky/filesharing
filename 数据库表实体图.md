# 数据库表实体图

本文档用于补充 [数据库表结构与ER图.md](数据库表结构与ER图.md) 和 [数据库表属性图.md](数据库表属性图.md)。这里采用“中心表名 + 周围字段”的方式，尽量贴近你给出的示意图风格。由于表数量较多，下面每张图只展示核心字段，完整字段说明仍建议以字段字典为准。图号按文档顺序递增。

## 1. 用户与文件核心表

### 图1 用户表实体图（users）

```mermaid
mindmap
  root((用户 users))
    id 主键ID
    username 用户名
    email 邮箱
    password 密码密文
    nickname 昵称
    avatar 头像URL
    storage_quota 存储配额
    used_storage 已用存储
    status 账户状态
    role 用户角色
    last_login_time 最后登录时间
    created_at 创建时间
    updated_at 更新时间
```

### 图2 文件夹表实体图（folders）

```mermaid
mindmap
  root((文件夹 folders))
    id 主键ID
    folder_name 文件夹名称
    name 兼容名称列
    description 描述
    parent_id 父文件夹ID
    owner_id 所有者ID
    is_public 是否公开
    folder_path 文件夹路径
    created_at 创建时间
    updated_at 更新时间
```

### 图3 文件表实体图（files）

```mermaid
mindmap
  root((文件 files))
    id 主键ID
    original_name 原始文件名
    storage_name 存储文件名
    file_path 文件路径
    file_size 文件大小
    content_type MIME类型
    extension 扩展名
    md5_hash MD5哈希
    status 文件状态
    is_public 是否公开
    uploader_id 上传者ID
    folder_id 所属文件夹ID
    created_at 创建时间
    updated_at 更新时间
    deleted_at 删除时间
```

### 图4 文件版本表实体图（file_versions）

```mermaid
mindmap
  root((文件版本 file_versions))
    id 主键ID
    file_id 关联文件ID
    version_number 版本号
    version_description 版本说明
    storage_name 存储名
    file_path 文件路径
    file_size 文件大小
    md5_hash MD5值
    content_type MIME类型
    modified_by 修改用户ID
    modified_at 修改时间
    is_current 是否当前版本
    is_deleted 是否已删除
    deleted_by 删除用户ID
    version_tag 版本标签
```

### 图5 文件统计表实体图（file_statistics）

```mermaid
mindmap
  root((文件统计 file_statistics))
    id 主键ID
    file_id 文件ID
    total_downloads 总下载次数
    total_previews 总预览次数
    total_shares 总分享次数
    unique_downloaders 唯一下载用户数
    unique_previewers 唯一预览用户数
    popularity_score 热度分
    last_download_time 最后下载时间
    last_preview_time 最后预览时间
    last_share_time 最后分享时间
    updated_at 更新时间
```

## 2. 标签与缩略图表

### 图6 标签表实体图（tags）

```mermaid
mindmap
  root((标签 tags))
    id 主键ID
    tag_name 标签名称
    color 标签颜色
    description 标签描述
    is_system_tag 是否系统标签
    created_by 创建人ID
    usage_count 使用次数
    created_at 创建时间
    updated_at 更新时间
```

### 图7 文件标签关联表实体图（file_tags）

```mermaid
mindmap
  root((文件标签 file_tags))
    id 主键ID
    file_id 文件ID
    tag_id 标签ID
    added_by 添加者ID
    added_at 添加时间
```

### 图8 文件夹标签关联表实体图（folder_tags）

```mermaid
mindmap
  root((文件夹标签 folder_tags))
    id 主键ID
    folder_id 文件夹ID
    tag_id 标签ID
    added_by 添加者ID
    added_at 添加时间
```

### 图9 缩略图表实体图（thumbnails）

```mermaid
mindmap
  root((缩略图 thumbnails))
    id 主键ID
    file_id 文件ID
    thumbnail_type 缩略图类型
    size_spec 尺寸规格
    thumbnail_path 缩略图路径
    thumbnail_name 缩略图名称
    file_size 文件大小
    width 宽度
    height 高度
    content_type MIME类型
    status 生成状态
    retry_count 重试次数
    max_retry_count 最大重试次数
    generated_at 生成时间
```

## 3. 分享与快传表

### 图10 分享记录表实体图（share_records）

```mermaid
mindmap
  root((分享 share_records))
    id 主键ID
    share_key 分享Key
    title 分享标题
    description 分享描述
    share_type 分享类型
    file_id 文件ID
    folder_id 文件夹ID
    sharer_id 分享者ID
    password 访问密码
    expire_time 过期时间
    max_access_count 最大访问次数
    current_access_count 当前访问次数
    pv_count PV
    uv_count UV
    status 分享状态
    allow_download 是否允许下载
    created_at 创建时间
```

### 图11 分享点击日志表实体图（share_click_logs）

```mermaid
mindmap
  root((分享点击 share_click_logs))
    id 主键ID
    share_id 分享记录ID
    share_key 分享Key
    visitor_ip 访客IP
    visitor_address 访客地址
    visitor_fingerprint 访客指纹
    user_agent 用户代理
    referer 来源页
    accessed_at 访问时间
```

### 图12 取件码表实体图（pickup_code_records）

```mermaid
mindmap
  root((取件码 pickup_code_records))
    id 主键ID
    code 取件码
    share_type 分享类型
    text_content 文本内容
    display_name 展示名称
    content_type 内容类型
    storage_path 存储路径
    storage_mode 存储模式
    cloud_config_id 云配置ID
    size_bytes 文件大小
    expire_at 过期时间
    used_count 已使用次数
    status 状态
    created_at 创建时间
    updated_at 更新时间
```

### 图13 旧版分享表实体图（shares）

```mermaid
mindmap
  root((旧版分享 shares))
    id 主键ID
    file_id 文件ID
    folder_id 文件夹ID
    share_key 分享密钥
    sharer_id 分享者ID
    share_type 分享类型
    expiration_time 过期时间
    access_count 访问次数
    max_access_count 最大访问次数
    password 访问密码
    created_at 创建时间
    updated_at 更新时间
```

## 4. 协作与内容表

### 图14 协作项目表实体图（collaboration_projects）

```mermaid
mindmap
  root((协作项目 collaboration_projects))
    id 主键ID
    project_name 项目名称
    description 项目描述
    owner_id 所有者ID
    status 项目状态
    is_public 是否公开
    cover_image 封面图
    tags 项目标签
    member_count 成员数
    file_count 文件数
    last_activity 最后活动时间
    created_at 创建时间
    updated_at 更新时间
```

### 图15 项目成员表实体图（project_members）

```mermaid
mindmap
  root((项目成员 project_members))
    id 主键ID
    project_id 项目ID
    user_id 用户ID
    role 成员角色
    status 成员状态
    invite_status 邀请状态
    invited_by 邀请人
    joined_at 加入时间
    last_activity 最后活动时间
    permissions 权限JSON
```

### 图16 协作文档表实体图（collaborative_documents）

```mermaid
mindmap
  root((协作文档 collaborative_documents))
    id 主键ID
    project_id 项目ID
    document_name 文档名称
    content 文档内容
    document_type 文档类型
    created_by 创建者
    last_edited_by 最后编辑者
    last_edited_at 最后编辑时间
    version 版本号
    is_locked 是否锁定
    locked_by 锁定用户
    locked_at 锁定时间
    view_count 查看次数
    comment_count 评论数
    status 文档状态
    created_at 创建时间
    updated_at 更新时间
```

### 图17 协作文档快照表实体图（collaborative_document_snapshots）

```mermaid
mindmap
  root((文档快照 collaborative_document_snapshots))
    id 主键ID
    document_id 文档ID
    version_number 版本号
    title 快照标题
    content 快照内容
    commit_message 提交说明
    created_by 创建者
    created_at 创建时间
```

### 图18 文档块表实体图（document_blocks）

```mermaid
mindmap
  root((文档块 document_blocks))
    id 主键ID
    document_id 文档ID
    content 块内容
    order_index 顺序号
    locked_by_user_id 锁定用户
    locked_at 锁定时间
    version 乐观锁版本
    created_at 创建时间
    updated_at 更新时间
```

### 图19 评论表实体图（comments）

```mermaid
mindmap
  root((评论 comments))
    id 主键ID
    project_id 项目ID
    file_id 文件ID
    author_id 作者ID
    content 评论内容
    parent_id 父评论ID
    comment_type 评论类型
    is_resolved 是否已解决
    resolved_at 解决时间
    resolved_by 解决人
    like_count 点赞数
    dislike_count 踩数
    created_at 创建时间
    updated_at 更新时间
```

### 图20 文档编辑者连接表实体图（document_editors）

```mermaid
mindmap
  root((文档编辑者 document_editors))
    document_id 文档ID
    user_id 用户ID
```

### 图21 文档标签表实体图（document_tags）

```mermaid
mindmap
  root((文档标签 document_tags))
    document_id 文档ID
    tag 标签文本
```

## 5. 上传、运维与统计表

### 图22 分片上传记录表实体图（chunk_upload_records）

```mermaid
mindmap
  root((分片上传 chunk_upload_records))
    id 主键ID
    upload_id 上传会话ID
    original_name 原始文件名
    file_size 文件总大小
    chunk_size 分片大小
    total_chunks 总分片数
    uploaded_chunks 已上传分片数
    status 上传状态
    uploader_id 上传者ID
    folder_id 目标文件夹ID
    md5_hash MD5哈希
    expire_time 过期时间
    created_at 创建时间
    updated_at 更新时间
```

### 图23 操作日志表实体图（operation_logs）

```mermaid
mindmap
  root((操作日志 operation_logs))
    id 主键ID
    operation_type 操作类型
    description 操作描述
    user_id 用户ID
    file_id 文件ID
    folder_id 文件夹ID
    share_id 分享ID
    ip_address IP地址
    user_agent 用户代理
    result 操作结果
    error_message 错误信息
    operation_time 操作时间
```

### 图24 通知表实体图（notifications）

```mermaid
mindmap
  root((通知 notifications))
    id 主键ID
    user_id 接收者ID
    title 标题
    content 内容
    notification_type 通知类型
    priority 优先级
    related_file_id 关联文件ID
    related_share_id 关联分享ID
    related_folder_id 关联文件夹ID
    is_read 是否已读
    is_sent 是否已发送
    send_channel 发送渠道
    sent_time 发送时间
    read_time 阅读时间
    expire_time 失效时间
    retry_count 重试次数
    max_retry_count 最大重试次数
    send_result 发送结果
    created_at 创建时间
    updated_at 更新时间
```

### 图25 通知模板表实体图（notification_templates）

```mermaid
mindmap
  root((通知模板 notification_templates))
    id 主键ID
    template_name 模板名称
    title_template 标题模板
    content_template 内容模板
    notification_type 通知类型
    is_enabled 是否启用
    supported_channels 支持渠道
    variables_description 变量说明
    created_by 创建人
    version 版本号
    created_at 创建时间
    updated_at 更新时间
```

### 图26 预览记录表实体图（preview_records）

```mermaid
mindmap
  root((预览记录 preview_records))
    id 主键ID
    file_id 文件ID
    user_id 用户ID
    preview_type 预览类型
    device_type 设备类型
    user_agent 用户代理
    ip_address IP地址
    duration_seconds 预览时长
    is_success 是否成功
    error_message 错误信息
    preview_time 预览时间
```

### 图27 搜索记录表实体图（search_records）

```mermaid
mindmap
  root((搜索记录 search_records))
    id 主键ID
    search_keyword 搜索关键词
    user_id 用户ID
    search_type 搜索类型
    result_count 结果数量
    search_duration 搜索耗时
    has_filters 是否有筛选条件
    filter_conditions 筛选条件JSON
    search_time 搜索时间
    client_ip 客户端IP
    user_agent 用户代理
```

### 图28 回收站表实体图（recycle_bin）

```mermaid
mindmap
  root((回收站 recycle_bin))
    id 主键ID
    item_id 被删项目ID
    item_type 项目类型
    original_name 原始名称
    original_path 原始路径
    original_parent_id 原始父级ID
    file_size 文件大小
    file_type 文件类型
    deleted_by 删除用户
    deleted_at 删除时间
    expire_at 过期时间
    is_recoverable 是否可恢复
    restore_target_id 恢复目标ID
    delete_reason 删除原因
    created_at 创建时间
```

### 图29 批量操作表实体图（batch_operations）

```mermaid
mindmap
  root((批量操作 batch_operations))
    id 主键ID
    operation_type 操作类型
    status 操作状态
    user_id 执行用户ID
    description 描述
    operation_params 操作参数JSON
    total_items 总项目数
    processed_items 已处理数
    success_items 成功数
    failed_items 失败数
    progress_percentage 进度
    is_cancellable 是否可取消
    started_at 开始时间
    completed_at 完成时间
    estimated_completion 预计完成时间
    created_at 创建时间
    updated_at 更新时间
```

### 图30 批量操作详情表实体图（batch_operation_details）

```mermaid
mindmap
  root((批量详情 batch_operation_details))
    id 主键ID
    batch_operation_id 批量操作ID
    item_id 项目ID
    item_type 项目类型
    item_name 项目名称
    item_status 处理状态
    result_message 结果消息
    error_message 错误信息
    started_at 开始时间
    completed_at 完成时间
    processing_time 处理耗时
    created_at 创建时间
```

### 图31 用户行为统计表实体图（user_behavior_stats）

```mermaid
mindmap
  root((用户行为统计 user_behavior_stats))
    id 主键ID
    user_id 用户ID
    total_uploads 总上传数
    total_downloads 总下载数
    total_previews 总预览数
    total_folders_created 总建文件夹数
    total_shares 总分享数
    total_storage_used 总使用量
    average_file_size 平均文件大小
    favorite_file_type 常用类型
    active_days 活跃天数
    last_active_time 最后活跃时间
    first_active_time 首次活跃时间
    user_level 用户等级
    user_points 用户积分
    period_start 周期开始
    period_end 周期结束
```

### 图32 系统配置表实体图（system_configs）

```mermaid
mindmap
  root((系统配置 system_configs))
    id 主键ID
    config_key 配置键
    config_value 配置值
    description 配置描述
    config_type 配置类型
    is_enabled 是否启用
    created_at 创建时间
    updated_at 更新时间
```

### 图33 系统统计表实体图（system_statistics）

```mermaid
mindmap
  root((系统统计 system_statistics))
    id 主键ID
    stat_date 统计日期
    total_users 总用户数
    new_users 新增用户数
    active_users 活跃用户数
    total_files 总文件数
    new_files 新增文件数
    total_folders 总文件夹数
    total_storage_used 总使用量
    total_storage_quota 总配额
    total_downloads 总下载数
    total_previews 总预览数
    total_shares 总分享数
    average_file_size 平均文件大小
    system_load 系统负载
    cpu_usage CPU使用率
    memory_usage 内存使用率
    disk_usage 磁盘使用率
    network_traffic 网络流量
    error_requests 错误请求数
    success_requests 成功请求数
    avg_response_time 平均响应时间
    created_at 创建时间
```

### 图34 智能推荐表实体图（smart_recommendations）

```mermaid
mindmap
  root((智能推荐 smart_recommendations))
    id 主键ID
    user_id 用户ID
    recommendation_type 推荐类型
    item_id 推荐项ID
    reason 推荐理由
    relevance_score 相关度
    source_type 来源类型
    source_model_id 来源模型ID
    is_viewed 是否查看
    is_adopted 是否采纳
    viewed_at 查看时间
    adopted_at 采纳时间
    created_at 创建时间
    expire_at 过期时间
```

### 图35 云存储配置表实体图（cloud_storage_configs）

```mermaid
mindmap
  root((云存储配置 cloud_storage_configs))
    id 主键ID
    config_name 配置名称
    provider_type 服务商类型
    access_key_id AccessKey
    access_key_secret Secret
    bucket_name 桶名
    region 区域
    custom_domain 自定义域名
    endpoint 端点
    base_path 基础路径
    is_enabled 是否启用
    is_default 是否默认
    storage_limit 容量限制
    used_storage 已用容量
    file_size_limit 文件大小限制
    allowed_file_types 允许类型
    connection_status 连接状态
    last_test_time 最后测试时间
    test_result 测试结果
    created_at 创建时间
    updated_at 更新时间
```

### 图36 云文件映射表实体图（cloud_file_mappings）

```mermaid
mindmap
  root((云文件映射 cloud_file_mappings))
    id 主键ID
    local_file_id 本地文件ID
    storage_config_id 存储配置ID
    cloud_key 云端Key
    cloud_url 云端URL
    etag ETag
    version_id 版本ID
    storage_class 存储类型
    is_uploaded 是否已上传
    uploaded_at 上传时间
    upload_attempts 上传次数
    max_retry_attempts 最大重试次数
    upload_error 上传错误
    cdn_enabled 是否启用CDN
    cdn_url CDN地址
    lifecycle_status 生命周期状态
    created_at 创建时间
```

### 图37 备份任务表实体图（backup_task）

```mermaid
mindmap
  root((备份任务 backup_task))
    task_id 任务ID
    backup_name 备份名称
    backup_type 备份类型
    status 任务状态
    start_time 开始时间
    end_time 结束时间
    success 是否成功
    error_message 错误信息
    db_backup_path 数据库备份路径
    files_backup_path 文件备份路径
    metadata_path 元数据路径
    backed_up_file_count 备份文件数
```

## 6. 说明

1. 这些实体图更适合论文、答辩材料和功能说明文档，展示方式接近你给出的示意图。
2. 如果你要，我可以继续把这些图拆成“每个模块一页”的排版版式，或者直接生成适合放入 Word/Markdown 的论文正文段落。
3. 如果你希望完全统一风格，我也可以把 [数据库表属性图.md](数据库表属性图.md) 和本文件统一改成相同的“实体图 + 字段字典”结构。
